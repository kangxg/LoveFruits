...
-(void)textFiledChange:(UITextField* )tf
{
    if (_MVAcounTf.text.length&&_MVPwdTf.text.length) {
        ...
        _MVLoginBtn.backgroundColor= [UIColor colorWithHexString:@"#ec4646"];
...
//             [[iToast makeText:@"商户信息审核中无法购物"] show];
            KGLogFailAlertView * alertview = [[KGLogFailAlertView alloc]init];
            [alertview setFailMeg:@"商户信息审核中无法购物"];
...
-(void)NTAlertViewCallBack:(id)sender WithTag:(NSInteger)tag
{
    ...
    if (alertView.tag == 100)
    {
        ...
        {
            //        KGLogauditAlertView  * view = sender;
...
-(UIButton *)MVForgetBtn
{
    if (_MVForgetBtn == nil)
    {
        ...
        [_MVForgetBtn setTitleColor:[UIColor colorWithHexString:@"#ffae02"] forState:UIControlStateNormal];
...
-(UIButton *)MVReginBtn
{
    if (_MVReginBtn == nil)
    {
        ...
        [_MVReginBtn setTitleColor:[UIColor colorWithHexString:@"#ffae02"] forState:UIControlStateNormal];
